...
%             stopWords = urlread(stopwordsURL);
            stopWords = fileread(stopwordsURL);
        ...
        function [tfidf,varargout] = getTFIDF(tokenized,dict)
            % initialize TDF matrix
            tdf = zeros(length(tokenized),length(dict));
            % loop over tokens from each note
            for i = 1:length(tokenized)
                bagOfWords = tokenized{i};
                % if bag of words is not empty
                if ~isempty(bagOfWords)
                    [uniqWords,~,idx] = unique(bagOfWords);
                    counts = accumarray(idx,1);
                    % get the indices of word_list for each word in words
                    [~,cols]=ismember(uniqWords,dict);
                    counts(cols==0) = [];
                    cols(cols==0) = [];
                    % update the matrix with the word counts for the row
                    tdf(i,cols) = counts;
                end
            end
            
            % compute TFIDF (Term Frequency x Inverse Document Frequency)
            N = length(tokenized);
            df = sum(tdf);
            idf = N./df;
            idf = log(idf);
            tfidf = bsxfun(@times,tdf,idf);
            
            % normalize tfidf with L2-norm
            l2norm = sqrt(sum(abs(tfidf).^2,2));
            tfidf = bsxfun(@rdivide,tfidf,l2norm);
            tfidf(isnan(tfidf)) = 0;
            varargout = {tdf};
        end
